import SeiunClient, { RequestOptions } from "./client";

export abstract class SeiunModule {
    protected client: SeiunClient;
    protected basePath: string;

    constructor(client: SeiunClient, basePath: string) {
        this.client = client;
    ...
    protected async get<T>(endpoint: string, options?: RequestOptions): Promise<T> {
        const response = await this.client.get<T>(this.getFullUrl(endpoint), options);
        return response.data;
    }

    // Post 方法
    protected async post<T>(endpoint: string, body: any, options?: RequestOptions): Promise<T> {
        const response = await this.client.post<T>(this.getFullUrl(endpoint), body, options);
        return response.data;
    }

    // Put 方法
    protected async put<T>(endpoint: string, body: any, options?: RequestOptions): Promise<T> {
        const response = await this.client.put<T>(this.getFullUrl(endpoint), body, options);
        return response.data;
    }

    // Delete 方法
    protected async delete<T>(endpoint: string, options?: RequestOptions): Promise<T> {
        const response = await this.client.delete<T>(this.getFullUrl(endpoint), options);
        return response.data;
    }
}